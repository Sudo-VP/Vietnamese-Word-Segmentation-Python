...
class Tokenizer:
    ...
    def tokenize(self,s):
        ...
        for token in tempTokens:
            ...
            for e in StringUtils().VN_abbreviation:
                ...
                tokens = self.recursive(tokens, token, i, i + e.length())
                break
            if tokenContainsAbb:
                continue
    ...
    def recursive( self,tokens, token,  beginMatch,  endMatch):
        if beginMatch > 0:
            for t in self.tokenize(token[0, beginMatch]):
                tokens.append(t)
        for t in self.tokenize(token[beginMatch, endMatch]):
                tokens.append(t)

        if endMatch < len(token):
            for t in self.tokenize(token[endMatch]):
                tokens.append(t)
